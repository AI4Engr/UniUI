...
- [x] The same business code runs in both Qt and Jupyter — Done (2026-08-30) for the *pages*: `admin_demo.py`'s `dashboard_page`/`users_page`/`settings_page`/`components_page` are now a single cross-backend implementation (the former `_browser_*_page` versions, renamed) used by Qt, Jupyter, and Web alike, with zero `PySide2`/`QtWidgets`/`QtCore`/`QtGui` references inside any `*_page` function (down from 59 references file-wide to 28, all confined to `main()`'s Qt-only shell). Ported the Qt dashboard's `TaskRunner` async refresh into the unified page (strictly better than the old synchronous browser refresh, and cross-backend). Along the way, found and fixed a genuine pre-existing Qt bug surfaced by using `create_wrap()` for the stat-card row: `_QFlowLayout` sized children via `QWidgetItem.sizeHint()`, which Qt zeroes out on any axis where the widget's size policy is `QSizePolicy.Ignored` (StatCard, Card, Table, MetricList labels all use `Ignored` horizontally) — every such child collapsed to width 0 inside a `Wrap` (`tests/test_qt_components_production.py::test_qt_wrap_sizes_ignored_policy_children_to_their_real_width`). Also fixed a users-page crash on Qt (`native_input.layout.width = ...` — `QWidget.layout` is a builtin method, not a settable ipywidgets-style trait, so the old `hasattr(..., "layout")` check crashed instead of no-op'ing on Qt). The **shell** (`create_admin_ui()`'s ~119-line browser build vs. `main()`'s ~205-line Qt build) is still split — genuinely harder/riskier (QSS stylesheet, `_ResponsiveTopBar`/`QToolBar`, icon rendering are legitimately Qt-only) and deliberately left for a separate change; see `notes/admin_demo_unification_plan.md` Step 3.
  - Partial follow-up (2026-08-31): eliminated every remaining hand-rolled QSS string in `main()`'s Qt shell (`_admin_stylesheet()` deleted entirely) by adding real library capabilities instead of demo-local workarounds — the `uniui-demo-*` → `uniui-page-*`/`uniui-shell-*` class rename, a Qt `add_class("uniui-icon-*")` → `QIcon` bridge (`QtButtonAdapter._apply_icon_class`, since Qt's QSS has no `mask-image`/`currentColor` equivalent to the CSS trick Web/Jupyter use), a new `ISeparator` primitive (`contracts/widgets.py`, alongside `IWrap`/`IGrid`), and a `tag_native()` helper for the one remaining raw `QWidget` subclass with no factory equivalent (`_ResponsiveTopBar`, needs a custom `resizeEvent`). This made the Qt shell's *styling* pipeline consistent with Jupyter/Web, but **did not** touch Step 3's actual scope: `main()`'s Qt shell is still a structurally separate ~180-line build (raw `QHBoxLayout`/`QVBoxLayout` composition) from `create_admin_ui()`'s shared `IAppShell`-based build used by Jupyter/Web. Real Step 3 remains open — see the new item directly below with a full brief for it.
  - **Step 3: done (2026-08-31)**. `main()`'s Qt shell and `create_admin_ui()` are now one function: `create_admin_ui()` is a strict superset covering every widget both paths need (search field, vertical separator, notification bell, and `header.on_resize(...)`-driven responsive collapse — all real cross-backend chrome now, not Qt-only extras), and `main()` shrank from ~200 lines to an 11-line entrypoint that just calls `create_admin_ui("qt")` and `show_ui(..., stylesheet=base_stylesheet())`, mirroring `_main_browser()`'s existing shape exactly. Turned out smaller in practice than the original brief scoped: no file split was needed (`admin_demo_shell.py`/`admin_demo_pages.py` etc.) since the merge fit cleanly in the existing two functions once the per-widget capability gaps (icon bridge, `ISeparator`, `set_leading_icon()`, `on_resize()` on `IHBoxLayout`) were already closed by prior work this same session.
    - `_NativeWrap` deleted entirely (was down to one use site, the footer, once the header moved to `create_hbox()` earlier) — the footer is now built with `f.create_hbox()`/`add_item()`/`add_stretch()` like every other cross-backend container, replacing raw `QtWidgets.QWidget()`/`QHBoxLayout` + manual `QSizePolicy` tweaks; verified empirically to lay out identically (left/right split via `add_stretch()`) before deleting the old code.
    - `_QT_RESTYLE_HOOK` still exists (Qt genuinely needs an explicit full-stylesheet repaint on theme change that Web/Jupyter don't — a `var()`/CSS-custom-property equivalent doesn't exist in Qt's QSS engine, confirmed by dedicated research this session) but is now assigned unconditionally from inside `create_admin_ui()`'s own `apply_theme` closure (guarded by `hasattr(shell.get_native(), "setStyleSheet")`, not a framework check), rather than living as a separate Qt-only closure duplicated in `main()`.
    - `uniui-web-status-ok`/`uniui-web-footer-meta` (the browser path's footer classes, pre-dating this session's `uniui-shell-*` convention) renamed to `uniui-shell-status-ok`/`uniui-shell-footer-meta` for consistency, with matching updates to `backends/jupyter/page_styles.py` and `backends/web/page_styles.py`.
    - Verified end-to-end on all three backends: full suite (1278 passed, no regressions), real Qt shell rendered via `main()`'s new path with responsive collapse checked at all three breakpoints and icon auto-retint confirmed via pixel sampling, and Jupyter/Web shells built via `create_admin_ui()` directly with the new search-field placeholder and separator element confirmed present in their native widget trees (not just "didn't crash").
- [x] Consistent dark/light theming with runtime toggle support — `theme_runtime.set_active_theme`, toggle wired in `admin_demo.py`, tested in `tests/test_theme_unification.py`
  - Footgun fixed (2026-08-31): `uniui.theme.set_theme`/`set_active_theme` (also reachable as top-level `uniui.set_theme`/`uniui.set_active_theme`) used to only flip the module-level `THEME` dict in place and never call the registered per-backend refresh hooks — no widget anywhere actually repainted, even though `uniui.theme_runtime.set_theme` (a same-named parallel function) worked correctly. Fixed by making `theme.py`'s own `set_active_theme` call a new `_notify_refresh()` (lazily importing `theme_runtime` to avoid a module-level circular import — documented as a fourth deferred-import case in `docs/architecture.md`), and `theme_runtime.set_theme`/`set_active_theme` became thin pass-throughs to `theme.py` so there's exactly one real implementation regardless of which module a caller reaches. Call-site audit confirmed zero tests asserted the old no-refresh behavior as correct and every backend's `set_admin_theme` compat shim was already correctly wired to `theme_runtime` — only `theme.py`'s own functions were lagging. New tests (`tests/test_theme_unification.py`) close the coverage gap that let this ship: one confirms the top-level export now fires registered refreshers, one confirms no double-fire across all three entry points (`uniui.set_theme`/`theme.set_theme`/`theme_runtime.set_theme`), one confirms a real Qt Admin component's `apply_theme()` actually fires via the top-level export (previously silent). Sabotage-verified.
  - Still open, deliberately not fixed in the same pass (different, larger blast radius — changes 3 examples' runtime behavior): `uniui.display.py::toggle_theme_and_refresh()` (used by `component_gallery.py`/`calculator.py`/`credit_card.py`) calls `theme.toggle_theme()` for state, then its own separate, weaker `refresh_theme(_root_widget)` — a basic single-root-widget tree walk that never touches `theme_runtime`'s registered refreshers, so Admin components (Card/Table/Sidebar, which repaint via `sync_palette()`) would not refresh through this path even after the fix above. Worth deciding whether these examples need it (do they use any Admin components today?) before wiring it up.
...
- [x] Fill in common capabilities for all components:
  ...
  - [x] `add_class()` / `remove_class()` — done (2026-08-30): real `IWidget` capability, default no-op, backed by NiceGUI classes (Web), ipywidgets DOM classes (Jupyter), and a boolean dynamic property + QSS attribute selector with `unpolish()`/`polish()` (Qt). See `notes/add_class_design_plan.md`. `IVBoxLayout`/`IHBoxLayout`/`IGrid` on Qt fall through to the inherited no-op (unlike `show`/`hide`/`set_enabled`/sizing on those three, which still raise `NotSupportedError`) since a bare `QLayout` has nothing to tag but Web's/Jupyter's equivalents are real, taggable elements.
    - Follow-up (2026-08-30): the shared page/typography vocabulary these classes style (`uniui-page-*`, `uniui-shell-*`) was originally named `uniui-demo-*` and lived only in `examples/admin_demo.py` + each backend's `demo_styles.py`, even though `backends/web/components/app_shell.py` and `backends/jupyter/components/app_shell_assets.py` already depended on it for real responsive-layout behavior — not demo-only. Renamed across all three backends; Qt gained the same page-CSS home Jupyter/Web already had (`backends/qt/primitives/styles.py::_page_css()`, folded into `base_stylesheet()`) instead of building QSS inline in the example. `demo_styles.py` renamed to `page_styles.py` in both backends to match.
...
- [x] Forbid backends from silently swallowing exceptions; errors must include backend, component type, and the original exception
  ...
  - Fixed (2026-08-31): `on_resize` (`IGrid` and `IHBoxLayout`) is now real on all three backends, not just Qt. Web: a hidden `ui.number()` bridge per instance + a browser-side `ResizeObserver` injected via `ui.run_javascript()` (guarded by `nicegui.core.is_loop_running()` so it's a safe no-op outside a live app), reporting width through the bridge's native `on_value_change` wiring. Jupyter: the same hidden-bridge-widget idiom the AppShell's sidebar splitter already used (`_width_bridge`/`SPLITTER_HTML`), but made automatic instead of pointer-driven — a `ResizeObserver` self-attaches once via `IPython.display.Javascript` injection at first `on_resize()` call, reaching the hidden bridge input through the same `closest()`/`querySelector()` DOM trick. **Explicitly experimental on both**: no automated test in this repo drives a real browser or notebook frontend, so only the Python-side edge-triggering logic (`mode != self._last_mode`, matching Qt's `_last_mode` gate exactly) is verified by test (sabotage-verified: breaking the gate makes the new tests fail as expected) — the actual `ResizeObserver`/DOM-traversal JS has not been exercised in a real browser or a real notebook (VS Code/JupyterLab/nbviewer/classic Notebook) in this session, and Jupyter in particular carries real cross-renderer risk (this codebase had no precedent for auto-injected-at-construction JS before this change — all prior JS injection was manually triggered). If the injected script never runs for any reason, `on_resize` silently never fires rather than crashing, matching the contract's existing no-op-is-acceptable philosophy.
...
- [ ] Add chart theme switching, empty/loading/error states, and responsive resize handling
  - Theme switching and empty/loading/error states are all done — `IChart.set_loading`/`set_error` exist (`components.py`) and are implemented + tested on all three backends (done 2026-08-24, see the P1 Chart Support section below). Still open: debounced resize handling specifically (see the Phase 4/Jupyter Responsive Renderer debounce lines below) — no `ResizeObserver`/resize-driven chart re-render exists at all yet.
- [x] Replace `QTableWidget` with `QTableView + QAbstractTableModel` for production DataGrid performance
  - Done (2026-09-01): `QtTableAdapter` now uses `QtWidgets.QTableView` backed by a new `_TableGridModel(QAbstractTableModel)` wrapping the existing `TableModel` by composition. Row click moved from `cellClicked` to `clicked(QModelIndex)`; refresh strategy is a full `beginResetModel`/`endResetModel()` (rows have no stable identity, so incremental `dataChanged` wasn't attempted — see the positional-diffing note earlier in this file). Alignment now comes from `data()`'s `TextAlignmentRole` instead of a per-item restyle loop.
- [x] Add delegates for status pills, numeric alignment, progress cells, and row action buttons
  - Done (2026-09-01): added `_ProgressCellDelegate` (paint-only, reuses `classify_status()`) and `_ActionButtonDelegate` (paint + `editorEvent()` hit-testing, not `setIndexWidget`, since per-cell `QWidget`s don't survive `_TableGridModel.refresh()`'s full reset) on Qt; `<progress>`/`<button>` HTML rendering plus a new string-valued bridge widget on Jupyter; conditional `body-cell-*` Quasar slots plus a `$parent.$emit`/`table.on()` bridge on Web. New explicit-opt-in column spec (`{"cell": "progress"}` / `{"cell": "actions", "actions": [...]}`) in `src/uniui/models/table.py`, and a new `ITable.on_row_action` contract method implemented on all three backends. Found and fixed a real Qt ghost-text rendering bug along the way (`_paint_cell_chrome()` now paints only `PE_PanelItemViewItem`, not `super().paint()`). Web/Jupyter's JS mechanisms are unverified against a live browser/notebook, same documented caveat as `on_resize()` above.
...
- [x] Add `Container`, `Spacer`, `Divider`
  - Done (2026-09-02): `Divider` deliberately not built as a separate widget — a dedicated research pass found zero distinct semantics from the existing `Separator`/`ISeparator` anywhere in this codebase's usage (no "labeled divider" concept exists); `Separator` already satisfies this need. `ISpacer(IWidget)` is a new standalone widget (`add_item()`-insertable into any container, unlike `add_stretch()` which only works on the box itself) — a bare expanding `QWidget` on Qt, a flex-grow `Box` on Jupyter, a `.grow` div on Web. Building it surfaced and fixed a real, previously-undetected bug: `JupyterVBoxAdapter.add_stretch()` was a silent no-op (`pass`) while `JupyterHBoxAdapter.add_stretch()` correctly inserted a spacer — both now share one `JupyterSpacer` class, and the existing `test_add_stretch` contract tests (which only checked "must not raise") gained a `test_add_stretch_actually_inserts_a_spacer` sibling on both VBox/HBox, sabotage-verified against the reverted bug. `IContainer(IWidget)` caps content width and centers it horizontally — closes a real cross-backend inconsistency where the private `.uniui-page` CSS mechanism had `max-width` only on Web (`1180px`, hardcoded), nothing on Jupyter, and no way to reach Qt at all (its class was tagged onto a classless `ILayoutOnly` VBox). Added a new `content_max_width` theme token (default `1180`, matching Web's prior hardcoded value) so all three backends' default now comes from one place instead of duplicated literals; `set_max_width()` overrides per-instance. New `TestSpacerContract`/`TestContainerContract` (`tests/contracts/test_layout.py`), contract-tested on all three backends.
...
- [x] Only reflow structure when crossing a breakpoint; delegate ordinary size changes to the native layout engine
  - Done (2026-08-31) for all three backends — see the detailed note under "Fill in common capabilities" above. Same `mode != self._last_mode` edge-trigger gate on Qt/Web/Jupyter alike; Web/Jupyter's real-browser/real-notebook behavior is unverified (Python-side logic only), flagged experimental.
...
- [x] `Badge` / `Tag`: status labels — `IBadge` (`components.py`), implemented on all three backends, reuses `models.status.classify_status()` (the same vocabulary `Table`'s status-pill column uses). Public `Badge(text, status="neutral")` in `facade.py`, demoed in `component_gallery.py`'s Data Display page.
- [x] `ProgressBar` — `IProgressBar` (`components.py`), implemented on all three backends (`backends/{qt,jupyter,web}/components/progress_bar.py`), already used by `component_gallery.py`'s Phase 1 pass and reused for Table's new progress-cell delegate above.
- [ ] `Table` / `DataGrid`:
  ...
  - [x] Single/multi-select — Done (2026-09-02). Multi-select is an explicit opt-in (`ITable.set_selection_mode("multiple")`), not Ctrl/Shift-click — it prepends a synthetic checkbox column (`SELECTION_COLUMN_KEY = "__uniui_selected__"`, `models/table.py`) that survives later `set_columns()` calls. `TableModel._selected_row` generalized to `_selected_rows: List[Dict]` (still by-value, still auto-dropped by `set_rows()` when a row disappears); `select_row()` always single-selects (row-click semantics are unchanged even in multiple mode — a checkbox toggle and a row click are independent gestures), new `toggle_row_selection()` is what the checkbox column drives. New `ITable.on_selection_change(fn)` fires in **both** modes (single-mode row clicks included), backed by a `TableModel._notify_selection()` equality gate (mirrors `State`'s own gate) so re-selecting the same row/toggling to the same set doesn't fire redundantly — including when `set_rows()` itself silently drops a stale selection, which also now dispatches (closed a gap the initial implementation missed: only click/checkbox paths dispatched at first). Qt: new `_CheckboxDelegate` (paint + `editorEvent()` hit-testing, same shape as the existing `_ActionButtonDelegate`). Jupyter: a new int-valued bridge widget, same idiom as the row-click bridge. Web: a new `q-checkbox` slot + `rowSelect` event bridge (same `$parent.$emit()`/`table.on()` mechanism as `rowAction`); needed an extra fix beyond the original design — `_formatted_rows()` stamps the checkbox boolean onto display rows for Quasar to read (`props.value`), and `_strip_checkbox_field()` strips it back off before any row reaches the model, since raw app rows never carry that key and by-value selection would otherwise silently break. `get_selected_row()` unchanged (`get_selected_rows()[0] if any else None`). 30+ new tests (model, contract — parametrized across all three `--ui` values, and per-backend direct tests including a Qt pixel-sampling visual check for checked vs. unchecked cells); sabotage-verified (the equality gate and the toggle's add/remove membership check both independently confirmed to break specific tests when disabled).
  ...
  - [x] Row action buttons — Done (2026-09-01), see the Table delegates entry under "Phase 2: Native Dashboard Components" above.
...
- [x] `RadioGroup` — Done (2026-08-29): `IRadioGroup` (`set_options(list[str])` selects the first automatically, `get_selected()`/`set_selected()`, `on_change()`), primitive-layer pattern like `Checkbox`/`Switch` (core form control, abstract on `IWidgetFactory`, not optional). Options are plain strings — both value and label, matching `IComboBox`/`IDropdown`'s `add_item(str)` convention. Qt: `QButtonGroup` + `QRadioButton`s composed in a plain `QWidget` (Qt has no single radio-group control, and `QButtonGroup` itself is a `QObject`, not a `QWidget`, with no show/hide/enabled/size surface). Jupyter/Web wrap native `ipywidgets.RadioButtons`/`ui.radio` directly — both required an explicit "select the first option" fix after `set_options()`, since neither auto-selects when `.options` is reassigned post-construction (confirmed empirically, not assumed).
- [x] `NumberInput` — Done (2026-08-29): `INumberInput` (`set_range()`/`set_step()`/`get_value()`/`set_value()`/`on_change()`), primitive-layer pattern like `Checkbox`/`Switch`/`RadioGroup`. Distinct from `ILineEdit.get_value()` (which parses arbitrary typed text and can raise `InvalidValueError`) — a NumberInput's value is always a valid, in-range float by construction. Qt: `QDoubleSpinBox` directly. Jupyter: `ipywidgets.BoundedFloatText` directly. Web: `ui.number`, with an explicit clamp-on-`set_range()` fix — NiceGUI's `.props()` only updates the client-side display bound, it does not clamp the current server-side `.value` the way Qt/ipywidgets do natively (confirmed empirically, not assumed), so the adapter clamps by hand to keep the contract identical across backends.
- [x] `Slider` — Done (2026-08-29): `ISlider`, the same `set_range()`/`set_step()`/`get_value()`/`set_value()`/`on_change()` contract as `NumberInput` for a dragged-track control instead of a typed/spun number. Qt's `QSlider` is integer-tick-only — `QtSliderAdapter` maps float minimum/maximum/step to an integer tick count (0..N) and back, so callers never see the tick scale; re-derives the tick range on every `set_range()`/`set_step()` call and re-applies the current value so it survives the rescale. Jupyter wraps `ipywidgets.FloatSlider` directly (auto-clamps like `BoundedFloatText`). Web wraps `ui.slider` with the same manual clamp-on-`set_range()` fix as `NumberInput`'s `ui.number` (confirmed empirically: `.props()` doesn't clamp `.value`).
- [ ] `DateInput`
...
- [ ] **Phase 2 — fill gaps one component at a time**, adding each to the gallery as it ships rather than waiting for all of them: `DateInput` (Inputs); `Avatar`, `List`, `Tree`, `Tag`, `Skeleton`, `Empty state` (Data Display — `Tag` may just be `Badge` restyled, check before building new); `Alert`, `Spinner`, `Confirmation` (Feedback); `Dialog`/`Modal` (blocking, unlike the existing non-modal `Drawer`), `Popover`, `Tooltip`, `Context Menu`, `Dropdown Menu` (Overlays — each needs a real `[Open ...]` trigger button in the gallery, not a screenshot); `Tabs` (may already be covered by `TabWidget`), `Menu`, `Pagination` (standalone), `Segmented Control` (Navigation); `Split View`/`Resizable Panel`/`Stack`/`Master-Detail` (Layout — check overlap with existing `SplitPane`/`Overlay` first).
  - [x] `RadioGroup` — Done (2026-08-29), see Forms and Feedback above. Wired into `examples/component_gallery.py`'s Inputs page with a real on_change-driven "Selected: X" echo label, not a screenshot.
  - [x] `NumberInput` — Done (2026-08-29), see Forms and Feedback above. Wired into `examples/component_gallery.py`'s Inputs page with a real on_change-driven "Value: X" echo label, not a screenshot.
  - [x] `Slider` — Done (2026-08-29), see Forms and Feedback above. Wired into `examples/component_gallery.py`'s Inputs page with a real on_change-driven "Value: X" echo label, not a screenshot.
...
- [x] Complete Jupyter Flex/Grid/container responsive renderer
  - Done (2026-08-31): `on_resize()` is real on Jupyter now (see the detailed note under "Fill in common capabilities" above) — an auto-injected `ResizeObserver` + hidden bridge-widget idiom, not Qt's `_ResizeNotifier` (that mechanism is Qt-specific, bound to `QLayout.setGeometry()`). Cross-notebook-frontend behavior is unverified and flagged experimental — no real notebook environment was available to exercise the injected JS.
- [ ] Pass the three-width cross-backend layout acceptance criteria before starting new complex business components — Qt's and the new Web/Jupyter Python-side edge-triggering logic are verified by test; the actual browser/notebook JS execution for Web/Jupyter is not (no automated test drives a real browser or notebook frontend)
...
- [x] `State` / `Computed` dependency updates, batching, unsubscribe, and cycle detection have unit tests
  - Dependency updates and unsubscribe/dispose are extensively tested (`tests/test_state.py`). Cycle detection (2026-08-22) and `uniui.batch()` (2026-08-24, `state.py`) are both implemented and tested — see the P0 State item above.
...
- [x] `Card` / `StatCard`
- [x] `Table` (static data, sorting, pagination) — static data, sorting, and pagination (`TableModel.set_page_size`/`set_page`/`page_count`, done 2026-08-24) all work and are tested.
- [x] `Sidebar` + `AppShell`
