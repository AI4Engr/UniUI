...
- [x] The same business code runs in both Qt and Jupyter — Done (2026-08-30) for the *pages*: `admin_demo.py`'s `dashboard_page`/`users_page`/`settings_page`/`components_page` are now a single cross-backend implementation (the former `_browser_*_page` versions, renamed) used by Qt, Jupyter, and Web alike, with zero `PySide2`/`QtWidgets`/`QtCore`/`QtGui` references inside any `*_page` function (down from 59 references file-wide to 28, all confined to `main()`'s Qt-only shell). Ported the Qt dashboard's `TaskRunner` async refresh into the unified page (strictly better than the old synchronous browser refresh, and cross-backend). Along the way, found and fixed a genuine pre-existing Qt bug surfaced by using `create_wrap()` for the stat-card row: `_QFlowLayout` sized children via `QWidgetItem.sizeHint()`, which Qt zeroes out on any axis where the widget's size policy is `QSizePolicy.Ignored` (StatCard, Card, Table, MetricList labels all use `Ignored` horizontally) — every such child collapsed to width 0 inside a `Wrap` (`tests/test_qt_components_production.py::test_qt_wrap_sizes_ignored_policy_children_to_their_real_width`). Also fixed a users-page crash on Qt (`native_input.layout.width = ...` — `QWidget.layout` is a builtin method, not a settable ipywidgets-style trait, so the old `hasattr(..., "layout")` check crashed instead of no-op'ing on Qt). The **shell** (`create_admin_ui()`'s ~119-line browser build vs. `main()`'s ~205-line Qt build) is still split — genuinely harder/riskier (QSS stylesheet, `_ResponsiveTopBar`/`QToolBar`, icon rendering are legitimately Qt-only) and deliberately left for a separate change; see `notes/admin_demo_unification_plan.md` Step 3.
  ...
  - **Step 3, full scope (not started)**: unify `main()`'s Qt shell construction with `create_admin_ui()` so there is exactly one shell-building code path for all three backends, the way page construction already is. A detailed external brief for this (received 2026-08-31, structure preserved close to verbatim) is queued here rather than executed immediately, since it's a multi-file, multi-day-scale refactor:
    1. Split `examples/admin_demo.py` into `admin_demo_data.py` (data/fixtures), `admin_demo_pages.py` (the already-unified page functions), `admin_demo_shell.py` (one shared shell builder), and a thin `admin_demo.py` entrypoint.
    2. `admin_demo_shell.py` builds header/breadcrumb/nav-buttons/search/theme-button/avatar/sidebar/content/footer using **only** UniUI factories and `IAppShell`-family APIs — no raw PySide2.
    3. Where Qt is missing a capability the shell needs, add it to the Qt **backend** (`backends/qt/components/`, `backends/qt/primitives/`, `backends/qt/display.py`) as a small, optional, backend-independent API — e.g. tooltip, icon, placeholder, compact/flat button styling, responsive visibility, footer alignment — rather than adding more raw-Qt code to the example. This is the same principle already applied this session for `add_class()`, the icon bridge, and `ISeparator`; Step 3 is really about extending it to cover the *shell container* itself (header/nav construction), not just individual widgets.
    4. Delete `_NativeWrap`, `_QT_RESTYLE_HOOK`, `_header_icon`, and the duplicated Qt header/footer construction in `main()` once the shared builder covers them; the example should have zero direct `PySide2`/`QtWidgets`/`QtCore`/`QtGui` imports at all (today confined to `main()`'s ~180 remaining lines).
    5. One theme controller/state flow for all three backends — no separate Qt-only global hook (`_QT_RESTYLE_HOOK`) sitting next to the browser path's `apply_theme`/`toggle`.
    6. Preserve exactly: `create_admin_ui("qt"/"jupyter"/"web")`'s public behavior, dark/light + named theme switching, sidebar/breadcrumb navigation, responsive/draggable shell behavior, existing CLI usage (`--ui qt/jupyter/web`).
    7. New/updated tests: all three backends build the same logical shell structure; no direct PySide2 import anywhere in `examples/admin_demo*.py` (grep-checkable); theme switching verified on all three backends; existing compat imports (`qt_components`/`jupyter_components`/`web_components`) still work.
    8. Full verification pass before considering it done: full suite, appearance-baseline tests, contract tests, an import-isolation check (no PySide2 import from the example), `py_compile` on changed files, `git diff --check` (whitespace), grep confirming zero PySide2 imports in the example.
    - Explicitly NOT a redesign of the UI or a deletion of any compat module — pure structural unification, matching everything else already done under this TODO item.
- [x] Consistent dark/light theming with runtime toggle support — `theme_runtime.set_active_theme`, toggle wired in `admin_demo.py`, tested in `tests/test_theme_unification.py`
  - Known footgun, not yet fixed (found 2026-08-31 while verifying an unrelated Qt icon-retint fix): `uniui.theme.set_theme(dark)` (also reachable as top-level `uniui.set_theme`) only flips the module-level `THEME` dict in place and never calls the registered per-backend refresh hooks — no widget anywhere actually repaints. The function that really works is `uniui.theme_runtime.set_theme(dark)`, which does notify every backend (`sync_palette()` on Qt, etc.). Two functions with the same name and very similar docstrings, only one of which does anything visible, is an easy trap for both users and future contributors — worth either making `theme.set_theme` delegate to `theme_runtime.set_theme` or renaming/deprecating one of them.
