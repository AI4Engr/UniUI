...
class WebChartAdapter(_WebAdapter, IChart):
    ...
    def _render(self) -> None:
        if self._model.shows_overlay:
            self._native.set_content(render_chart_message_svg(self._model.overlay_text(), get_palette()))
        else:
            self._native.set_content(render_chart_svg(*self._model.render_args(), get_palette()))
